...
LatLonCounts2 = LatLonCounts[which(LatLonCounts$Freq > 0),]
summary(LatLonCounts2)
...
# Quick Question
ggplot(murderMap, aes(x = long, y = lat, group = group, fill = GunOwnership)) + geom_polygon(color = "black") + scale_fill_gradient(low = "black", high = "red", guide = "legend", limits = c(0,.6))
summary(murderMap$GunOwnership)
murderMap$region[which.max(murderMap$GunOwnership)]
